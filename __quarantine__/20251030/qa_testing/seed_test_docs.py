...
Inserts 2 test documents with embeddings using text-embedding-3-small
"""
...
# Add the backend directory to Python path to import rag modules
sys.path.insert(0, '/app/backend-minimal')

from rag.llm import get_embedding

def seed_test_documents():
    """Seed 2 test documents with embeddings"""
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            for doc in test_docs:
                ...
                # Generate embedding using existing LLM client
                try:
                    embedding = get_embedding(doc['content'])
                    if not embedding or len(embedding) != 1536:
                        print(f"❌ Invalid embedding received for {doc['source']}")
                        return False
                    print(f"✅ Generated embedding ({len(embedding)} dimensions)")
                except Exception as e:
                    print(f"❌ Embedding generation failed for {doc['source']}: {e}")
                    return False
            ...
            # Commit all changes
            conn.commit()
            print(f"\n✅ Successfully seeded {len(test_docs)} documents with embeddings")
...
if __name__ == "__main__":
    ...
    print("🎯 Target: 2 test documents with embeddings")
    ...
    if success:
        ...
        print("Ready for end-to-end RAG testing")
    else:
        print("\n❌ SEEDING FAILED")
