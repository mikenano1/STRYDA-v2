#!/usr/bin/env python3
"""
Mammoth Insulation Deep Dive - Download and Process Pipeline
Downloads PDFs from Supabase Storage organized by document type
"""

import os
import sys
import json
import time
from datetime import datetime

sys.path.insert(0, '/app/backend-minimal')
sys.path.insert(0, '/app/data/processing')

from dotenv import load_dotenv
load_dotenv('/app/backend-minimal/.env')

from supabase import create_client

# Configuration
SUPABASE_URL = os.getenv('SUPABASE_URL')
SUPABASE_KEY = os.getenv('SUPABASE_SERVICE_ROLE_KEY')
BASE_PATH = 'C_Interiors/Mammoth'
LOCAL_DIR = '/app/data/ingestion/C_Interiors/Mammoth/downloads'
OUTPUT_DIR = '/app/data/processing/mammoth_deep_dive'

# Folder to doc_type mapping (user-provided categorization)
FOLDER_DOC_TYPES = {
    '1_Data_Sheets': 'Technical_Data_Sheet',
    '2_Certificates': 'Certification',
    '3_Install_Guides': 'Installation_Guide',
    '4_BPIR': 'BPIR_Declaration',
}

# Trade classification rules for Mammoth products
MAMMOTH_TRADE_RULES = {
    'wall_insulation': {
        'keywords': ['wall', 'wall section', 'wall insulation', 'interior wall', 'stud'],
        'product_family': 'Mammoth Wall'
    },
    'ceiling_insulation': {
        'keywords': ['ceiling', 'ceiling blanket', 'ceiling insulation', 'overhead'],
        'product_family': 'Mammoth Ceiling'
    },
    'underfloor_insulation': {
        'keywords': ['underfloor', 'floor', 'subfloor', 'carpark', 'suspended floor'],
        'product_family': 'Mammoth Underfloor'
    },
    'roof_insulation': {
        'keywords': ['roof', 'skillion', 'rafter', 'pitched roof'],
        'product_family': 'Mammoth Roof'
    },
    'acoustic_insulation': {
        'keywords': ['acoustic', 'sound', 'noise', 'baffle', 'absorber', 'panel absorber',
                     'soundproofing', 'noise control', 'stc', 'rw', 'nrc', 'deco'],
        'product_family': 'Mammoth Acoustic'
    },
    'duct_insulation': {
        'keywords': ['duct', 'hvac', 'liner', 'duct liner', 'mechanical'],
        'product_family': 'Mammoth Duct'
    },
    'general_insulation': {
        'keywords': ['thermal', 'polyester', 'insulation', 'r-value', 'blanket'],
        'product_family': 'Mammoth General'
    },
}


def classify_document(filename: str, folder: str, content: str = '') -> dict:
    """Classify document by trade and doc_type based on filename, folder, and content"""
    text = (filename + ' ' + content[:2000]).lower()
    
    # Doc type from folder (user pre-categorized)
    doc_type = FOLDER_DOC_TYPES.get(folder, 'Technical_Manual')
    
    # Detect trade
    trade = 'general_insulation'
    product_family = 'Mammoth General'
    best_score = 0
    
    for trade_key, rules in MAMMOTH_TRADE_RULES.items():
        score = sum(1 for kw in rules['keywords'] if kw in text)
        if score > best_score:
            best_score = score
            trade = trade_key
            product_family = rules['product_family']
    
    return {
        'trade': trade,
        'product_family': product_family,
        'doc_type': doc_type
    }


def download_files():
    """Download all PDFs from Supabase Storage"""
    print("\n" + "="*60)
    print("📥 DOWNLOADING MAMMOTH PDFs FROM SUPABASE")
    print("="*60)
    
    supabase = create_client(SUPABASE_URL, SUPABASE_KEY)
    os.makedirs(LOCAL_DIR, exist_ok=True)
    
    downloaded = []
    
    for folder in FOLDER_DOC_TYPES.keys():
        folder_path = f"{BASE_PATH}/{folder}"
        local_folder = os.path.join(LOCAL_DIR, folder)
        os.makedirs(local_folder, exist_ok=True)
        
        # List files in folder
        files = supabase.storage.from_('product-library').list(folder_path)
        pdf_files = [f for f in files if f.get('name', '').endswith('.pdf') and f.get('name') != '_placeholder.pdf']
        
        print(f"\n📂 {folder}: {len(pdf_files)} files")
        
        for f in pdf_files:
            filename = f['name']
            remote_path = f"{folder_path}/{filename}"
            local_path = os.path.join(local_folder, filename)
            
            # Skip if already downloaded
            if os.path.exists(local_path):
                print(f"   ⏭️ Already exists: {filename}")
                downloaded.append((local_path, folder))
                continue
            
            try:
                print(f"   ⬇️ Downloading: {filename}")
                response = supabase.storage.from_('product-library').download(remote_path)
                
                with open(local_path, 'wb') as file:
                    file.write(response)
                
                downloaded.append((local_path, folder))
                time.sleep(0.2)
                
            except Exception as e:
                print(f"      ❌ Error: {e}")
    
    print(f"\n✅ Downloaded {len(downloaded)} files")
    return downloaded


def process_pdfs(pdf_files: list):
    """Process PDFs through Vision-enabled chunker"""
    print("\n" + "="*60)
    print("🔬 PROCESSING PDFs WITH VISION PIPELINE")
    print("="*60)
    
    from pdf_chunker_v2 import VisionPDFChunker
    
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    chunker = VisionPDFChunker(output_dir=OUTPUT_DIR, enable_vision=True)
    
    all_chunks = []
    file_stats = []
    
    for i, (pdf_path, folder) in enumerate(pdf_files, 1):
        filename = os.path.basename(pdf_path)
        print(f"\n[{i}/{len(pdf_files)}] 📄 Processing: {filename}")
        print(f"    Category: {folder}")
        
        try:
            # Process PDF
            chunks = chunker.process_pdf(pdf_path)
            
            # Classify each chunk using folder info
            classification = classify_document(filename, folder)
            
            for chunk in chunks:
                chunk_dict = chunk.to_dict() if hasattr(chunk, 'to_dict') else chunk
                chunk_dict['trade'] = classification['trade']
                chunk_dict['product_family'] = classification['product_family']
                chunk_dict['doc_type'] = classification['doc_type']
                chunk_dict['brand_name'] = 'Mammoth'
                chunk_dict['category_code'] = 'C_Interiors'
                # Update source to include brand
                chunk_dict['source'] = f"Mammoth Deep Dive - {filename}"
                all_chunks.append(chunk_dict)
            
            file_stats.append({
                'filename': filename,
                'folder': folder,
                'chunks': len(chunks),
                'trade': classification['trade'],
                'doc_type': classification['doc_type']
            })
            
            print(f"    ✅ {len(chunks)} chunks | trade={classification['trade']} | doc_type={classification['doc_type']}")
            
        except Exception as e:
            print(f"    ❌ Error: {e}")
            file_stats.append({
                'filename': filename,
                'folder': folder,
                'chunks': 0,
                'error': str(e)
            })
    
    # Save all chunks
    chunks_file = os.path.join(OUTPUT_DIR, 'all_chunks.json')
    with open(chunks_file, 'w') as f:
        json.dump(all_chunks, f, indent=2, default=str)
    
    # Save stats
    stats_file = os.path.join(OUTPUT_DIR, 'processing_stats.json')
    stats = {
        'timestamp': datetime.now().isoformat(),
        'total_files': len(pdf_files),
        'total_chunks': len(all_chunks),
        'chunker_stats': chunker.stats,
        'file_stats': file_stats,
        'trade_distribution': {},
        'doc_type_distribution': {},
    }
    
    for chunk in all_chunks:
        trade = chunk.get('trade', 'unknown')
        doc_type = chunk.get('doc_type', 'unknown')
        stats['trade_distribution'][trade] = stats['trade_distribution'].get(trade, 0) + 1
        stats['doc_type_distribution'][doc_type] = stats['doc_type_distribution'].get(doc_type, 0) + 1
    
    with open(stats_file, 'w') as f:
        json.dump(stats, f, indent=2)
    
    print(f"\n" + "="*60)
    print("📊 PROCESSING SUMMARY")
    print("="*60)
    print(f"   Total files processed: {len(pdf_files)}")
    print(f"   Total chunks created: {len(all_chunks)}")
    print(f"   Images found: {chunker.stats.get('images_found', 0)}")
    print(f"   Images captioned: {chunker.stats.get('images_captioned', 0)}")
    print(f"   Tables extracted: {chunker.stats.get('tables_extracted', 0)}")
    print(f"\n   Trade Distribution:")
    for trade, count in sorted(stats['trade_distribution'].items(), key=lambda x: -x[1]):
        print(f"      • {trade}: {count}")
    print(f"\n   Doc Type Distribution:")
    for doc_type, count in sorted(stats['doc_type_distribution'].items(), key=lambda x: -x[1]):
        print(f"      • {doc_type}: {count}")
    
    print(f"\n   📁 Output saved to: {OUTPUT_DIR}")
    
    return all_chunks, stats


def ingest_to_database(chunks: list):
    """Ingest chunks directly to database with embeddings"""
    print("\n" + "="*60)
    print("💾 INGESTING TO DATABASE")
    print("="*60)
    
    import psycopg2
    import psycopg2.extras
    from openai import OpenAI
    
    DATABASE_URL = 'postgresql://postgres.qxqisgjhbjwvoxsjibes:8skmVOJbMyaQHyQl@aws-1-ap-southeast-2.pooler.supabase.com:5432/postgres'
    
    # Get API key for embeddings
    from emergentintegrations.llm.openai import get_openai_client
    client = get_openai_client(os.getenv('EMERGENT_LLM_KEY'))
    
    conn = psycopg2.connect(DATABASE_URL, sslmode='require')
    
    batch_size = 20
    total_inserted = 0
    
    for i in range(0, len(chunks), batch_size):
        batch = chunks[i:i+batch_size]
        print(f"\n📦 Batch {i//batch_size + 1}/{(len(chunks) + batch_size - 1)//batch_size} ({len(batch)} chunks)...")
        
        # Generate embeddings for batch
        texts = [c.get('content', '')[:8000] for c in batch]
        
        try:
            response = client.embeddings.create(
                model="text-embedding-3-small",
                input=texts
            )
            embeddings = [e.embedding for e in response.data]
        except Exception as e:
            print(f"   ❌ Embedding error: {e}")
            continue
        
        # Insert into database
        with conn.cursor() as cur:
            for j, chunk in enumerate(batch):
                try:
                    cur.execute("""
                        INSERT INTO documents (
                            source, content, page, doc_type, trade, brand_name,
                            product_family, category_code, priority, embedding, phase
                        ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                    """, (
                        chunk.get('source', 'Mammoth Deep Dive'),
                        chunk.get('content', ''),
                        chunk.get('page', 1),
                        chunk.get('doc_type', 'Technical_Manual'),
                        chunk.get('trade', 'general_insulation'),
                        'Mammoth',
                        chunk.get('product_family', 'Mammoth General'),
                        'C_Interiors',
                        85,
                        embeddings[j],
                        'mammoth_deep_dive'
                    ))
                    total_inserted += 1
                except Exception as e:
                    print(f"   ❌ Insert error: {e}")
        
        conn.commit()
        time.sleep(0.5)
    
    conn.close()
    print(f"\n✅ Inserted {total_inserted} chunks into database")
    return total_inserted


def main():
    print("\n" + "="*70)
    print("🦣 MAMMOTH INSULATION DEEP DIVE - INGESTION PIPELINE")
    print("="*70)
    
    # Step 1: Download files
    pdf_files = download_files()
    
    if not pdf_files:
        print("❌ No PDF files found to process")
        return
    
    # Step 2: Process through Vision pipeline
    chunks, stats = process_pdfs(pdf_files)
    
    # Step 3: Ingest to database
    if chunks:
        ingest_to_database(chunks)
    
    print("\n" + "="*70)
    print("✅ MAMMOTH DEEP DIVE COMPLETE")
    print("="*70)


if __name__ == "__main__":
    main()
